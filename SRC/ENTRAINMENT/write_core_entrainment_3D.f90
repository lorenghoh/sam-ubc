subroutine write_core_entrainment_3D
...
if(masterproc.or.output_sep) then
  ...
  if(RUN3D) then
    ...
    filename='./OUT_3D/'//trim(case)//'_CORE_ENTRAIN_'//trim(caseid)//'_'// &
...
! JTD
  nfields1=nfields1+1
  tmp = (core_values(1:nx, 1:ny, 1:nzm, 3) - &
         old_core_values(1:nx, 1:ny, 1:nzm, 3))/dt
  name='DWDT'
  long_name='Vertical Velocity Tendency'
  units='m/s2'
  call compress3D(tmp,nx,ny,nzm,name,long_name,units, &
                                 save3Dbin,dompi,rank,nsubdomains)

  nfields1=nfields1+1
  tmp = volume_core(1:nx, 1:ny, 1:nzm)
  name='VTETCOR'
  long_name='Tetrahedral Core Volume'
  units='m3/m3'
  call compress3D(tmp,nx,ny,nzm,name,long_name,units, &
                                 save3Dbin,dompi,rank,nsubdomains)

  nfields1=nfields1+1
  tmp = w_adv_fluxes(1:nx, 1:ny, 1:nzm, 1)*w_area_core_frac(1:nx, 1:ny, 1:nzm)
  name='MFTETCOR'
  ...
  nfields1=nfields1+1
  tmp = E_tetra_core(1:nx, 1:ny, 1:nzm, 1)
  name='ETETCOR'
  long_name='Tetrahedral Core Mass Entrainment'
  units='kg/m3/s'
  call compress3D(tmp,nx,ny,nzm,name,long_name,units, &
                                 save3Dbin,dompi,rank,nsubdomains)

  nfields1=nfields1+1
  tmp = D_tetra_core(1:nx, 1:ny, 1:nzm, 1)
  name='DTETCOR'
  long_name='Tetrahedral Core Mass Detrainment'
  units='kg/m3/s'
  call compress3D(tmp,nx,ny,nzm,name,long_name,units, &
                                  save3Dbin,dompi,rank,nsubdomains)

  nfields1=nfields1+1
  tmp = E_tetra_core(1:nx, 1:ny, 1:nzm, 2)
  name='EQTETCOR'
  long_name='Tetrahedral Core Specific Humidity Entrainment'
  units='kg/m3/s'
  call compress3D(tmp,nx,ny,nzm,name,long_name,units, &
                                  save3Dbin,dompi,rank,nsubdomains)
 
  nfields1=nfields1+1
  tmp = D_tetra_core(1:nx, 1:ny, 1:nzm, 2)
  name='DQTETCOR'
  long_name='Tetrahedral Core Specific Humidity Detrainment'
  units='kg/m3/s'
  call compress3D(tmp,nx,ny,nzm,name,long_name,units, &
                                  save3Dbin,dompi,rank,nsubdomains)

  nfields1=nfields1+1
  tmp = E_tetra_core(1:nx, 1:ny, 1:nzm, 3)
  name='ETTETCOR'
  long_name='Tetrahedral Core Liquid Water Moist Static Energy Entrainment'
  units='K kg/m3/s'
  call compress3D(tmp,nx,ny,nzm,name,long_name,units, &
                                  save3Dbin,dompi,rank,nsubdomains)
  
  nfields1=nfields1+1
  tmp = D_tetra_core(1:nx, 1:ny, 1:nzm, 3)
  name='DTTETCOR'
  long_name='Tetrahedral Core Liquid Water Moist Static Energy Detrainment'
  units='K kg/m3/s'
  call compress3D(tmp,nx,ny,nzm,name,long_name,units, &
                                  save3Dbin,dompi,rank,nsubdomains)

  nfields1=nfields1+1
  tmp = E_tetra_core(1:nx, 1:ny, 1:nzm, 4)
  name='EWTETCOR'
  long_name='Tetrahedral Core Vertical Velocity Entrainment'
  units='m/s kg/m3/s'
  call compress3D(tmp,nx,ny,nzm,name,long_name,units, &
                                  save3Dbin,dompi,rank,nsubdomains)

  nfields1=nfields1+1
  tmp = D_tetra_core(1:nx, 1:ny, 1:nzm, 4)
  name='DWTETCOR'
  long_name='Tetrahedral Core Vertical Velocity Detrainment'
  units='m/s kg/m3/s'
  call compress3D(tmp,nx,ny,nzm,name,long_name,units, &
                                  save3Dbin,dompi,rank,nsubdomains)
